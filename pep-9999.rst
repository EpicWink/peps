...
All of the :pep:`508` environment marker variables are available, with the
following mapping (period ``.`` indicates that the part to the right is a key
in the table with name as the part to the left).
...
Note that the ``extra`` marker is missing: this is only to be specified as
``for-extra`` in ``optional-dependencies`` requirements.
...
.. code-block:: python
    ...
    def convert_requirement_to_pep508(name, requirement):
        ...
        for variable, key1, key2 in (
            ...
            if key2 in requirement.get(key1, {}):
                markers.append(variable + " " + requirement[key1][key2])
        if "for-extra" in requirement:
            markers.append("extra = '" + requirement["for-extra"] + "'")
